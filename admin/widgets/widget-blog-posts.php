...
/*
	...
	function of_Blog_Posts_Widget() {
		...
		$widget_ops = array(
			'classname' => 'posts',
			'description' => __('A widget that displays the blog posts.', 'of_blog_posts_widget')
		);
		...
		// Create the widget
		$this->__construct( 'of_blog_posts_widget', __('# Blog Posts', 'of_blog_posts_widget'), $widget_ops );
		...
		<?php else: ?>
			...
			<?php if ( have_posts() ) : while ( have_posts() ) : the_post(); global $post; ?>
				...
				<div <?php post_class(); ?>>
					...
					<p class="byline">
							<?php printf( __('<span class="text">On</span> %2$s <span class="text">by</span> %1$s', 'of_blog_posts_widget'), '<span class="author vcard"><a class="url fn n" href="' . get_author_posts_url( get_the_author_meta('ID') ) . '" title="' . get_the_author() . '">' . get_the_author() . '</a></span>', '<abbr class="published" title="' . sprintf( get_the_time( __('l, F jS, Y, g:i a', 'of_blog_posts_widget') ) ) . '">' . sprintf( get_the_time( __('F j, Y', 'of_blog_posts_widget') ) ) . '</abbr>' ); ?>
						</p>
					...
					<div class="entry-summary" style="line-height: 21px">
						<?php
						...
						?>
						<?php /* if get the full image from media library only. The new way is actually pretty elegant and efficient. */ ?>
